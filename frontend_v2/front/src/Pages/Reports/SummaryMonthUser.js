...
const SummaryMonthUserTable = () => {
  ...
  const sortData = (data) => {
    return [...data].sort((a, b) => {
      ...
      } else if (sortConfig.key === 'aloc_all_calls') {
        return sortConfig.direction === 'asc' 
          ? a.aloc_all_calls - b.aloc_all_calls 
          : b.aloc_all_calls - a.aloc_all_calls
      }
      return 0
    })
  }
                    ...
                    <tbody>
                      {pagedData.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td>{formatSessionTime(item.aloc_all_calls)}</td>
                          <td>{item.nbcall}</td>
                          <td>{item.nbcall_fail || 0}</td>
